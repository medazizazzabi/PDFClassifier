...
def settingDirectory(output):
    ...
    if not os.path.exists(output+"/val/labels"):
        os.makedirs(output+"/val/labels")
...
def splitDataset(dataset,output,train,test):
    ...
    test_images = random.sample([file for file in files if file not in train_images],num_test)
    val_images = [file for file in files if file not in train_images and file not in test_images]
    ...
    for image in val_images:
        os.rename(dataset+"/images/"+image+".png",output+"/val/images/"+image+".png")
        os.rename(dataset+"/labels/"+image+".txt",output+"/val/labels/"+image+".txt")
...
if __name__ == "__main__":
    ...
    try:
        ...
        exit(1)
    try:
        val = parser.add_argument('-val', '--val', help='The validation dataset',type=int)
    except:
        print("Please specify the validation percentage")
        exit(1)
